...
export default function ManageAdmins() {
  ...
  useEffect(() => {
    ...
    setCurrentPage(1);
  }, [searchTerm, filterStatus, admins, sortField, sortDirection]);
  ...
  const confirmStatusChange = () => {
    if (pendingStatusChange) {
      updateAdminStatus(pendingStatusChange.id, pendingStatusChange.status);
    }
  ...
  const confirmDelete = async () => {
    ...
    try {
      await adminApi.deleteAdmin(pendingDeleteId);
                  ...
                  {currentItems.length > 0 ? (
                    currentItems.map((admin, index) => (
                      ...
                      >
                        <td className="py-4 px-4">
                          <div className="flex items-center">
                            ...
                            <div>
                              ...
                              <div className="text-xs text-muted-foreground dark:text-foreground/70">ID: {admin.id}</div>
                            </div>
                          </div>
                        </td>
                        <td className="py-4 px-4 text-sm text-foreground/90 dark:text-foreground/90">{admin.email}</td>
                        <td className="py-4 px-4">
                        ...
                        <td className="py-4 px-4">
                          <div className="relative">
                            <AlertDialog open={statusChangeDialogOpen && pendingStatusChange?.id === admin.id} onOpenChange={(open) => {
                              if (!open) {
                                setStatusChangeDialogOpen(false);
                                setPendingStatusChange(null);
                              }
                              ...
                              <AlertDialogContent>
                                <AlertDialogHeader>
                                  ...
                                  <AlertDialogDescription>
                                    Are you sure you want to change the status of <strong>{admins.find(a => a.id === pendingStatusChange?.id)?.name}</strong> to <strong>{pendingStatusChange?.status}</strong>? This action can be reverted later.
                                  </AlertDialogDescription>
                                </AlertDialogHeader>
                                <AlertDialogFooter>
                              ...
                              <SelectTrigger className={`h-9 w-[130px] border-none shadow-none focus:ring-0 capitalize ${admin.status === 'active' ? 'text-emerald-400 bg-emerald-500/10' :
                                ...
                                <div className="flex items-center">
                                  {updatingStatus === admin.id ? (
                                    ...
                                    <div className={`h-2 w-2 rounded-full mr-2 ${admin.status === 'active' ? 'bg-emerald-400 animate-pulse' :
                                      admin.status === 'suspended' ? 'bg-amber-400' :
                                        'bg-gray-400'
                                      }`} />
                                  )}
                                  <SelectValue />
                        ...
                        <td className="py-4 px-4">
                          <div className="flex items-center space-x-1">
                            ...
                            <div className="relative">
                              <AlertDialog open={deleteDialogOpen && pendingDeleteId === admin.id} onOpenChange={(open) => {
                                if (!open) {
                                  setDeleteDialogOpen(false);
                                  setPendingDeleteId(null);
                                }
                    ...
                    <tr>
                      <td colSpan={5} className="py-16 px-4 text-center">
                        <div className="flex flex-col items-center justify-center">
                          ...
                          <p className="text-muted-foreground/90 text-lg font-medium">No admins found</p>
                  ...
                  >
                    <div className="flex justify-between items-start">
                      <div className="flex items-center">
                        ...
                        <div className="overflow-hidden">
                          <h3 className="font-bold text-foreground truncate max-w-[180px]">{admin.name}</h3>
                          <p className="text-xs text-foreground/60">ID: {admin.id}</p>
                        </div>
                      </div>
                    ...
                    <div className="grid grid-cols-2 gap-4 text-sm">
                      <div className="space-y-1">
                        ...
                        <p className="text-foreground/90 truncate">{admin.email}</p>
                      </div>
                      <div className="space-y-1">
                        <p className="text-muted-foreground text-xs uppercase tracking-wider font-semibold">Role</p>
                        <p className="text-foreground/90 capitalize">{admin.role}</p>
                      </div>
                    </div>
              ...
              <div className="p-4 sm:p-6 border-t border-border flex flex-col sm:flex-row items-center justify-between gap-4">
                <div className="flex flex-col sm:flex-row items-center gap-4">
                  <div className="text-sm text-center sm:text-left text-foreground/80">
                    ...
                    <span className="font-medium text-foreground">{Math.min(indexOfLastItem, filteredAdmins.length)}</span> of{' '}
                    <span className="font-medium text-foreground">{filteredAdmins.length}</span> results
                  </div>
