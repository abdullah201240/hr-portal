...
export default function CompanyLayout({
    ...
    useEffect(() => {
        console.log('CompanyLayout - Auth State:', { isLoading, isAuthenticated });
        if (!isLoading && !isAuthenticated) {
            console.log('CompanyLayout - Redirecting to login because not authenticated');
        ...
        <div className="flex min-h-screen flex-col bg-background text-foreground relative overflow-hidden">
            ...
            <div className="flex flex-1 relative z-10">
                ...
                <div className={`flex flex-col flex-1 transition-all duration-300 ${sidebarCollapsed ? 'md:ml-16' : 'md:ml-64'}`}>
                    ...
                    {/* Footer */}
                    <CompanyFooter className={`fixed bottom-0 right-0 z-50 ${sidebarCollapsed ? 'md:left-16' : 'md:left-64'} left-0 transition-all duration-300`} />
                </div>
            </div>
        </div>
    );
